...
from keras.layers import Conv3D, GlobalAveragePooling3D, AveragePooling3D
...
def convolution_block(x, nb_channels, dropout_rate=None, bottleneck=False, weight_decay=1e-4):
    ...
    if bottleneck:
        ...
        x = BatchNormalization(gamma_regularizer=l2(weight_decay), beta_regularizer=l2(weight_decay))(x)
        x = Activation('relu')(x)
        x = Conv3D(nb_channels * bottleneckWidth, (1, 1, 1), use_bias=False, kernel_regularizer=l2(weight_decay))(x)
        # Dropout
        if dropout_rate:
    ...
    x = BatchNormalization(gamma_regularizer=l2(weight_decay), beta_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
    x = Conv3D(nb_channels, (3, 3, 3), padding='same', use_bias=False, kernel_regularizer=l2(weight_decay))(x)
...
def transition_layer(x, nb_channels, dropout_rate=None, compression=1.0, weight_decay=1e-4):
    ...
    x = BatchNormalization(gamma_regularizer=l2(weight_decay), beta_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
    x = Conv3D(int(nb_channels*compression), (1, 1, 1), padding='same',
